...
from app.services.task_urgency import normalize_task_urgency, task_urgency_rank
...
- Определяй срочность: "срочно", "критично", "ASAP", "важно" -> urgent;
  по умолчанию и для "не горит", "когда будет время", "по возможности" -> normal.
...
- Определи срочность: "срочно", "важно", "критично", "ASAP" -> urgent;
  "когда будет время", "не горит" и обычные задачи -> normal.
...
class AIService:
    ...
    def _merge_parsed_chunks(cls, chunks: list[ParsedMeeting]) -> ParsedMeeting:
        ...
        for chunk in chunks:
            ...
            for task in chunk.tasks:
                task.priority = normalize_task_urgency(task.priority)
    ...
    async def parse_task_from_text(
        ...
        try:
            data = self._extract_json(response)
            parsed = ParsedVoiceTask(**data)
            parsed.priority = normalize_task_urgency(parsed.priority)
            return parsed
        except (json.JSONDecodeError, ValueError) as e:
