...
class GetCourseService:
    ...
    def _parse_decimal(raw: Any) -> Decimal:
        ...
        Handles whitespace separators (including non-breaking spaces),
        comma as decimal separator, and trailing non-numeric characters
        (currency symbols, units) — common in Russian-locale exports.
        """
        if not raw:
            return Decimal("0")
        ...
        try:
            ...
            pass
        # Fallback: strip all non-numeric characters (currency symbols, etc.)
        stripped = re.sub(r"[^\d.\-]", "", clean).strip(".")
    ...
    def _sum_payments(rows: list) -> tuple[int, Decimal]:
        ...
        for i, row in enumerate(rows):
            if not isinstance(row, list):
                ...
                if i == 0:
                    logger.warning(
                        "Payment row[0] is %s, not list: %r",
                        type(row).__name__, row,
                    )
                continue
            if PAYMENT_PRICE_INDEX >= len(row):
                skipped_short += 1
                if i == 0:
                    logger.warning(
                        "Payment row[0] too short (len=%d, need>%d): %r",
                        len(row), PAYMENT_PRICE_INDEX, row,
                    )
                continue
            val = GetCourseService._parse_decimal(row[PAYMENT_PRICE_INDEX])
            if i == 0:
                logger.info(
                    "Payment row[0]: len=%d, raw[%d]=%r, parsed=%s, full_row=%r",
                    len(row), PAYMENT_PRICE_INDEX, row[PAYMENT_PRICE_INDEX],
                    val, row,
                )
            total += val

        if skipped_short or skipped_nonlist:
            logger.warning(
                "Payment rows: total=%d, skipped_short=%d, skipped_nonlist=%d",
                count, skipped_short, skipped_nonlist,
            )
        if count > 0 and total == 0:
            logger.warning(
                "Payment sum is 0 for %d rows — possible column index mismatch "
                "(PAYMENT_PRICE_INDEX=%d)",
                count, PAYMENT_PRICE_INDEX,
            )
    ...
    def _sum_orders(rows: list) -> tuple[int, Decimal]:
        ...
        for row in rows:
            ...
            if not logged_first:
                logger.info(
                    "Order row[0]: len=%d, raw[%d]=%r",
                    len(row), ORDER_SUM_INDEX, row[ORDER_SUM_INDEX],
                )
                logged_first = True
        ...
        if skipped_short:
            logger.warning(
                "Order rows: total=%d, skipped_short=%d", total_rows, skipped_short,
            )
