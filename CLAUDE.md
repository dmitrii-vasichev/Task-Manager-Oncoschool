...
Таск-менеджер для команды ~20 человек с двумя рабочими точками входа: Telegram-бот и веб-интерфейс.
...
План Telegram Mini App (архив/legacy для задачного потока): **MINI_APP_PLAN.md**
...
- **Telegram задачи:** inline-only flow (`/tasks` + callback-кнопки), Mini App для задач отключён.
...
### Telegram Mini App (MINI_APP_PLAN.md) — legacy архив ✅
...
### Telegram Mini App (legacy)

| Фаза | Как проверить |
|------|---------------|
| MA1 | POST /api/auth/mini-app возвращает 410 Gone, /app в боте сообщает про inline `/tasks` |
...
| MA5 | Legacy-проверка Mini App: исторически были Menu Button/тема Telegram; текущий задачный поток inline-only |
