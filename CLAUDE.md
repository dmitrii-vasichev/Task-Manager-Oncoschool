...
### База данных
- PostgreSQL (Supabase)
- 10 таблиц: team_members, tasks, task_updates, meetings, meeting_participants, notification_subscriptions, reminder_settings, app_settings, meeting_schedules, telegram_notification_targets
...
- [ ] Фаза T1: Миграция БД + Модели (departments, team_members поля, роли)
- [ ] Фаза T2: Backend API + Permissions (departments CRUD, avatar upload, tree endpoint)
- [ ] Фаза T3: Бот + совместимость (3 роли, /start, /help)
- [ ] Фаза T4: Frontend типы + API + хуки (types, api, permissions, RoleBadge)
- [ ] Фаза T5: Дерево + карточки (TeamTree, DepartmentNode, MemberCard, модальные окна)
- [ ] Фаза T6: Полировка + совместимость (sidebar, avatar everywhere, CLAUDE.md)
