...
function taskCountForm(
  ...
  if (mod10 === 1 && mod100 !== 11) return `${count} ${forms.one}`;
...
export function formatTaskPublishSummaryCount(count: number): string {
  return taskCountForm(count, {
    one: "задача",
    few: "задачи",
    many: "задач",
  });
...
export function formatTaskPublishActionCount(count: number): string {
  return taskCountForm(count, {
    one: "задачу",
    few: "задачи",
    many: "задач",
